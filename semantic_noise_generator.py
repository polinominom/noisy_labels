...
def log_needed_chunks(lst,p='',t='train'):
    ...
    for gi in lst:
        ...
        if not ci in needed_chunk_ids.keys():
            needed_chunk_ids[ci]=0
        else:
            needed_chunk_ids[ci]+=1
    
    s = sorted(needed_chunk_ids.items(), key=lambda kv: kv[1])
    msg = '%s_%s: %s'%(str(p), str(t), str(s))
    with open('./_%s_%s_chunk_list.txt'%(str(p),str(t)),'w+') as fp:
        fp.writelines(msg)
    print('length: %i'%len(s))
