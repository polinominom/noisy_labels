...
opts, args = getopt.getopt(sys.argv[1:], "l:n:c:b:")
noise = None
loss = None
epoch_resume = None
batch_size = None
...
if loss == 'est_forward':
    saved_model_path = fpath('./models', 'crossentropy', noise)+'_latest.h5'
    ...
    kerasModel.load_model(saved_model_path)
    # estimate P
...
# test
test_loader = utils.get_covid_test_loader(batch_size)

# some additional callbacks
prediction_save_folder = f'./network_training_predictions/test_covid_forwardt_{loss}_{int(noise*100)}'
...
pcs = PredictionSaveCallback(None, test_loader, h5_save=None, prediction_folder=prediction_save_folder, epoch_resume=epoch_resume)
pcs.batch_size = test_loader.get_total_item_count()
callbacks = [pcs]

score = kerasModel.evaluate_model(test_loader, callbacks=callbacks)
