...
parser.add_argument('--noise_folder', type=str, help='the global path to the folder that contains all generated noises...')
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        # todo 3: get real loss, real acc
        train_loss, train_acc, train_real_loss, train_real_acc  = train(epoch, trainloader, net, criterion, optimizer)
        test_loss, test_acc, test_real_loss, test_real_acc      = test(epoch, testloader, net, criterion)
...
def train(epoch, trainloader, net, criterion, optimizer):
    ...
    if (epoch+1) >= args.start_prune and (epoch+1) % 10 == 0:
        ...
        now = torch.load('./checkpoint/current_net.h5')
        net = now['current_net']
        net.train()
    # todo 4: get the real ground truths and get the real acc and real loss. 
...
def test(epoch, testloader, net, criterion):
    ...
    torch.save(state, './checkpoint/current_net.h5')
...
def checkpoint(acc, epoch, net):
    ...
    torch.save(state, './checkpoint/ckpt.t7.' + args.sess)
